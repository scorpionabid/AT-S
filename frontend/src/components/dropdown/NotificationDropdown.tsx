...
const NotificationDropdown: React.FC<NotificationDropdownProps> = ({ className = '' }) => {
  ...
  return (
    <div className={`relative inline-block ${className}`} ref={dropdownRef}>
      {/* Notification Trigger - Styled to match other header action buttons */}
      <button
        ...
        title="Bildirişlər"
        className={`relative p-2 rounded-lg transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-2 dark:focus:ring-offset-neutral-900 text-neutral-600 hover:bg-neutral-100 dark:text-neutral-300 dark:hover:bg-neutral-800 h-10 w-10 flex items-center justify-center ${className}`}
      >
        <FiBell size={20} />
        {unreadCount > 0 && (
      ...
      {/* Dropdown Menu */}
      {isOpen && (
        <div className="absolute top-full right-0 w-80 bg-white rounded-lg shadow-xl border border-gray-200 z-60 mt-2 overflow-y-auto max-h-96" role="menu" style={{ maxHeight: 'calc(100vh - 80px)' }}>
          {/* Header */}
